   ...
   const [name, setName] = useState('');
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`https://homeessential-fdca5e469865.herokuapp.com/api/v1/auth/ligsingle/${params.id}`);
                if (!response.ok) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let response = await fetch(`https://homeessential-fdca5e469865.herokuapp.com/api/v1/auth/ligsingle/${params.id}`, {
                method: "PUT",
                body: formData
            });
