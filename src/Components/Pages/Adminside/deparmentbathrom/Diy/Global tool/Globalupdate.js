...
const Globalupdate = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`http://srv577826.hstgr.cloud:8002/api/v1/auth/globalsingle/${params.id}`);
        ...
        fetchData();
            ...
            const updateuser = async (e)=>{
                ...
                try {
                    let result = await fetch(`http://srv577826.hstgr.cloud:8002/api/v1/auth/globalsingle/${params.id}` , {
        method: "put",
        body :formdata
