...
const Umbrellaform = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let result = await fetch("https://homeessential-fdca5e469865.herokuapp.com/api/v1/auth/umbrella2products", {
        method: "POST",
        body: formData,
      });
      result = await result.json();
      if (result) {
        navigate("/listumbrella2");
  ...
  return (
    <div>
      <Link to="/listumbrella2" className="btn btn-success mss mt-2">Product-list</Link>
      <form className='marr'>
