...
const Hotwaterform = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let result = await fetch("http://srv577826.hstgr.cloud:8002/api/v1/auth/hotwaterproducts", {
        method: "POST",
        body: formData,
      });
      result = await result.json();
      if (result) {
        navigate("/listhotwater");
  ...
  return (
    <div>
      <Link to="/listhotwater" className="btn btn-success mss mt-2">Product-list</Link>
      <form className='marr'>
