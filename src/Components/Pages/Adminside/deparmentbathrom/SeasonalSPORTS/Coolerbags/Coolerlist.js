...
//         let result = await fetch("https://api.homeessentialshive.co.uk/api/v1/data/sportuser")
...
//           // console.log(id)
//           try {
//               let result= await fetch(`https://api.homeessentialshive.co.uk/api/v1/data/sportuserid/${id}`,{
//             method:"delete"
...
const Coolerlist = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('https://api.homeessentialshive.co.uk/api/v1/data/sportuser');
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`https://api.homeessentialshive.co.uk/api/v1/data/sportuserid/${id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <StaticListComponent
      ...
      onEdit={handleEdit}
      editUrlBase='/listcooler'  // Set the base URL for editing
      addProductUrl='/coolerform'  // Set the URL for adding a new product
    />
  );
};
