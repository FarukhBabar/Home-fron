...
//     }
//     try {
//       let result = await fetch("https://homeessential-fdca5e469865.herokuapp.com/api/v1/auth/disposibleproducts" , {
//         method: "post",
//         body :formdata
...
//       <Link to="/listdisposible" className="btn btn-success mss mt-2" >Product-list</Link>
//       <form className='marr '>
...
// export default Showheadform;


import React, { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import StaticForm from '../../../../FormPage1';
const Disposibleform = () => {
  const [name, setName] = useState('');
    const [title, setTitle] = useState('');
    const [price, setPrice] = useState('');
    const [image, setImage] = useState(null);
const navigate = useNavigate();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let response = await fetch('https://homeessential-fdca5e469865.herokuapp.com/api/v1/auth/disposibleproducts', {
                method: 'POST',
                body: formData
            });
    ...
    return (
        <StaticForm
            ...
            listUrl="/listdisposible" 
        />
    );
};

export default Disposibleform
