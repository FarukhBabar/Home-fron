...
//         let result = await fetch("https://api.homeessentialshive.co.uk/api/v1/data/singlestemuser")
...
//           // console.log(id)
//           try {
//               let result= await fetch(`https://api.homeessentialshive.co.uk/api/v1/data/singlestemuserid/${id}`,{
//             method:"delete"
...
const Singlestemlist = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('https://api.homeessentialshive.co.uk/api/v1/data/singlestemuser');
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`https://api.homeessentialshive.co.uk/api/v1/data/singlestemuserid/${id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <StaticListComponent
      ...
      onEdit={handleEdit}
      editUrlBase='/listsinglestem'  // Set the base URL for editing
      addProductUrl='/singlestemform'  // Set the URL for adding a new product
    />
  );
};
