...
//         let result = await fetch("https://api.homeessentialshive.co.uk/api/v1/data/birdsuser")
...
//           // console.log(id)
//           try {
//               let result= await fetch(`https://api.homeessentialshive.co.uk/api/v1/data/birdsuserid/${id}`,{
//             method:"delete"
...
const Birdslist = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('https://api.homeessentialshive.co.uk/api/v1/data/birdsuser');
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`https://api.homeessentialshive.co.uk/api/v1/data/birdsuserid/${id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <StaticListComponent
      ...
      onEdit={handleEdit}
      editUrlBase='/listbirds'  // Set the base URL for editing
      addProductUrl='/birdsform'  // Set the URL for adding a new product
    />
  );
};
