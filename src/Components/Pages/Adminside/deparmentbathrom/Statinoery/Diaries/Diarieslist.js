...
//         let result = await fetch("https://api.homeessentialshive.co.uk/api/v1/data/diariesuser")
...
//           // console.log(id)
//           try {
//               let result= await fetch(`https://api.homeessentialshive.co.uk/api/v1/data/diariesuserid/${id}`,{
//             method:"delete"
...
const Diarieslist = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('https://api.homeessentialshive.co.uk/api/v1/data/diariesuser');
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`https://api.homeessentialshive.co.uk/api/v1/data/diariesuserid/${id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <StaticListComponent
      ...
      onEdit={handleEdit}
      editUrlBase='/listdiaries'  // Set the base URL for editing
      addProductUrl='/diariesform'  // Set the URL for adding a new product
    />
  );
};
