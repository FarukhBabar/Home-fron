...
const Metalbuckform = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let result = await fetch("http://srv577826.hstgr.cloud:8001/api/v1/auth/metalbuckproducts", {
        method: "POST",
        body: formData,
      });
      result = await result.json();
      if (result) {
        navigate("/listmetalbuck");
  ...
  return (
    <div>
      <Link to="/listmetalbuck" className="btn btn-success mss mt-2">Product-list</Link>
      <form className='marr'>
