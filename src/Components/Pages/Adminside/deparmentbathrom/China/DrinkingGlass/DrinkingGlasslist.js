...
//         let result = await fetch("https://api.homeessentialshive.co.uk/api/v1/data/drinkingglassuser")
...
//           // console.log(id)
//           try {
//               let result= await fetch(`https://api.homeessentialshive.co.uk/api/v1/data/drinkingglassuserid/${id}`,{
//             method:"delete"
...
const DrinkingGlasslist = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('https://api.homeessentialshive.co.uk/api/v1/data/drinkingglassuser');
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`https://api.homeessentialshive.co.uk/api/v1/data/drinkingglassuserid/${id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <StaticListComponent
      ...
      onEdit={handleEdit}
      editUrlBase='/listdrinkingglass'  // Set the base URL for editing
      addProductUrl='/drinkingglassform'  // Set the URL for adding a new product
    />
  );
};
