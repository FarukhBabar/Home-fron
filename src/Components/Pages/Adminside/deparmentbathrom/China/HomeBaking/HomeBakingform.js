...
const HomeBakingform = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let result = await fetch("http://srv577826.hstgr.cloud:8001/api/v1/auth/homebakingproducts", {
        method: "POST",
        body: formData,
      });
      result = await result.json();
      if (result) {
        navigate("/listhomebaking");
  ...
  return (
    <div>
      <Link to="/listhomebaking" className="btn btn-success mss mt-2">Product-list</Link>
      <form className='marr'>
