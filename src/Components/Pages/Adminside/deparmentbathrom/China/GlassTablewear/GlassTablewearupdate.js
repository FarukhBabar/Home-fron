...
const GlassTablewearupdate = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`http://srv577826.hstgr.cloud:8002/api/v1/auth/glasstablewaersingle/${params.id}`);
    ...
    const updateUser = async (e) => {
        ...
        try {
            let response = await fetch(`http://srv577826.hstgr.cloud:8002/api/v1/auth/glasstablewaersingle/${params.id}`, {
                method: "PUT",
                body: formData
            });
