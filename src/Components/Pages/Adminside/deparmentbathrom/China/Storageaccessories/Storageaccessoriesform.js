...
const Storageaccessoriesform = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let result = await fetch("https://homeessential-fdca5e469865.herokuapp.com/api/v1/auth/storageaccessproducts", {
        method: "POST",
        body: formData,
      });
      result = await result.json();
      if (result) {
        navigate("/liststorageaccessories");
  ...
  return (
    <div>
      <Link to="/liststorageaccessories" className="btn btn-success mss mt-2">Product-list</Link>
      <form className='marr'>
...
export default Storageaccessoriesform;
