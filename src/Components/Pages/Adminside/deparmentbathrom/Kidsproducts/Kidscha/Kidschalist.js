...
const Kidschalist = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('http://srv577826.hstgr.cloud:8002/api/v1/data/newuser');
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`http://srv577826.hstgr.cloud:8002/api/v1/data/newuserid/${id}`, {
        method: 'DELETE',
      });
