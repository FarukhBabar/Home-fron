...
const Listofproducts = () => {
  ...
  const handleDelete = async () => {
    try {
      let result = await fetch(`https://homeessential-fdca5e469865.herokuapp.com/api/v1/data/userid/${selectedUserId}`, {
        method: 'DELETE',
      });
      result = await result.json();
      if (result) {
  ...
  return (
    <Container className='my-4'>
      <h4 className='text-center text-warning display-4 fw-bold'>List of SundayOffer Products</h4>
      <Link to='/adminsunday'>
        ...
        <tbody>
          {user.map((ele, ind) => (
            <tr key={ele._id}>
              ...
              <td>
                <Link to={`/list/${ele._id}`} className='btn btn-success me-2'>
