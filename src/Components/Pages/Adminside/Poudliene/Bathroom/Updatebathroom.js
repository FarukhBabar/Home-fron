   ...
   const [name, setName] = useState('');
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`http://srv577826.hstgr.cloud:8002/api/v1/auth/bathromsingle/${params.id}`);
                if (!response.ok) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let response = await fetch(`http://srv577826.hstgr.cloud:8002/api/v1/auth/bathromsingle/${params.id}`, {
                method: "PUT",
                body: formData
            });
