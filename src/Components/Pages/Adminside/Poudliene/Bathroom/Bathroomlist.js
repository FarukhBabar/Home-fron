...
const Bathroomlist = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('https://homeessential-fdca5e469865.herokuapp.com/api/v1/data/bathuser');
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`https://homeessential-fdca5e469865.herokuapp.com/api/v1/data/bathuserid/${id}`, {
        method: 'DELETE',
      });
