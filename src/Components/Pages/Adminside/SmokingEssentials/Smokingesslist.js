...
const Smokingesslist = () => {
  ...
  const setUsersData = async () => {
    let result = await fetch('http://srv577826.hstgr.cloud:8002/api/v1/data/Smokuser');
  ...
  const handleDelete = async () => {
    try {
      let result = await fetch(`http://srv577826.hstgr.cloud:8002/api/v1/data/Smokuserid/${selectedUserId}`, {
        method: 'DELETE',
      });
      result = await result.json();
      if (result) {
