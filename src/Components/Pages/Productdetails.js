...
//       try {
//         const response = await axios.get(`https://homeessential-fdca5e469865.herokuapp.com/api/products/most-selling/${id}`);
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const response = await axios.get(`https://homeessential-fdca5e469865.herokuapp.com/api/products/most-selling/${formattedName}`);
        setProduct(response.data);
      } catch (error) {
        console.error('Error fetching product details:', error);
      } finally {
